...
const warningKey = (method, params) =>
  `${method}:${params?.document?.uri || params?.source?.uri || "workspace"}`;

const warnOnce = (session, method, params, title, detail) => {
  if (session && typeof session === "object") {
    let seen = warnings.get(session);
    if (!seen) warnings.set(session, (seen = new Set()));
    const key = warningKey(method, params);
